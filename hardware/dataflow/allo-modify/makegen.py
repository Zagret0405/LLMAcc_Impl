...
def generate_makefile(
    desc_file, path, platform="vitis_hls", hbm_mapping=None, auto_hbm_mapping=False
):
    ...
    # Store mapping-related metadata and re-write the description file
    # so sub-generators can access it.
    if hbm_mapping is not None or auto_hbm_mapping:
        if auto_hbm_mapping:
            data["auto_hbm_mapping"] = True
        else:
            data.pop("auto_hbm_mapping", None)

    if hbm_mapping is not None:
        data["hbm_mapping"] = hbm_mapping
    elif "hbm_mapping" in data:
        data.pop("hbm_mapping", None)
