...
from .vitis import (
    ...
    extract_hls_m_axi_arg_names,
    build_round_robin_hbm_mapping,
)
from .pynq import (
    ...
    def __init__(
        ...
        with Context() as ctx, Location.unknown():
            ...
            pm = PassManager.parse(
                "builtin.module(empty-tensor-to-alloc-tensor,func.func(convert-linalg-to-affine-loops))"
            )
            pm.run(self.module.operation)
            if hasattr(allo_d, "lower_memcopy_ops"):
                if not allo_d.lower_memcopy_ops(self.module):
                    raise RuntimeError("Failed to lower memref.copy operations")
            else:
                # Keep fallback fully textual so it works with old bindings.
                PassManager.parse("builtin.module(copy-on-write,memref-dce)").run(
                    self.module.operation
                )
                if "memref.copy" in str(self.module):
                    raise RuntimeError(
                        "`memref.copy` remains after fallback cleanup. "
                        "Your runtime is missing `lower-memcopy-ops`; "
                        "please rebuild/reinstall Allo MLIR bindings."
                    )
        buf = io.StringIO()
        success = True
        match platform:
        ...
        if project is not None:
            ...
            if self.platform == "vitis_hls":
                ...
                device_name = str(configs.get("device", "")).lower()
                auto_hbm_mapping = (
                    hbm_mapping is None
                    and self.mode == "hw_emu"
                    and "u280" in device_name
                )
                generate_makefile(
                    dst_path,
                    project,
                    self.platform,
                    hbm_mapping,
                    auto_hbm_mapping=auto_hbm_mapping,
                )
                ...
                elif auto_hbm_mapping:
                    hls_m_axi_args = extract_hls_m_axi_arg_names(
                        self.hls_code, self.top_func_name
                    )
                    if hls_m_axi_args:
                        cfg_content = generate_hbm_config(
                            self.top_func_name,
                            build_round_robin_hbm_mapping(hls_m_axi_args),
                        )
                        cfg_path = os.path.join(project, f"{self.top_func_name}.cfg")
                        with open(cfg_path, "w", encoding="utf-8") as cfg_file:
                            cfg_file.write(cfg_content)
